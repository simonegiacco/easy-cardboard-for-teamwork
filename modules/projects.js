	...
	module.controller('ProjectsCtrl', [ '$scope', 'ViewService', 'RestService', '$http', '$routeParams', 'ProjectCreateDialogService',
			function($scope, ViewService, RestService, $http, $routeParams, ProjectCreateDialogService) {
				...
				$scope.saveProject = function() {
					...
					$scope.projectNew.startDate = $scope.parseDate($scope.projectNew.startDate);
					$scope.projectNew.endDate = $scope.parseDate($scope.projectNew.endDate);
				...
				$scope.showProjectDialog = function(projectId) {
					...
					if (!projectId) {
						...
						$http({
							...
							$scope.projectNew.startDate = $scope.getFormattedDate(data.project.startDate);
							$scope.projectNew.endDate = $scope.getFormattedDate(data.project.endDate);							
							ProjectCreateDialogService.show();
						}).error(function(data, status, headers, cfg) {
						});
					}
